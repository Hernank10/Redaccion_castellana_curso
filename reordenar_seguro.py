...
for course_id, course_name in courses:
    # Paso 1: Asignar un order temporal = id + 100000 (para evitar colisiones)
    cursor.execute("UPDATE core_lesson SET \"order\" = id + 100000 WHERE course_id = ?", (course_id,))
    conn.commit()
    
    # Paso 2: Reasignar order secuencial (1,2,3,...)
